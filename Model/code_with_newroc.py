...
from amlm_trial import mlm_loss_patient, load_data_from_pkl,TransformerWithConv

#############################################
# New functions for AUROC calculation
#############################################

def calculate_auprc_55(predictions_55, labels_55):
    """
    Calculate micro-AUPRC and macro-AUPRC for 55 class-based multi-hot predictions.
    ...
    Returns:
        A tuple (micro_auprc, macro_auprc) with the calculated AUPRC scores.
    """
    import numpy as np
    from sklearn.metrics import average_precision_score
    ...
    # Compute micro-average by flattening the arrays.
    micro_auprc = average_precision_score(y_true.ravel(), y_pred.ravel())

    # For macro average, compute AP for each valid class.
    valid_scores = []
    for j in range(y_true.shape[1]):
        unique_vals = np.unique(y_true[:, j])
        if len(unique_vals) > 1:
            score = average_precision_score(y_true[:, j], y_pred[:, j])
            valid_scores.append(score)
    if len(valid_scores) > 0:
        macro_auprc = np.mean(valid_scores)
    else:
        macro_auprc = None
    ...
    if macro_auprc is not None:
        ...
        print("Macro-AUPRC: Undefined (no valid classes)")
...
def evaluate_model(patient_visits, patient_notes, time_diffs, patient_labels, code_map, vocab, d, device, 
                    base_encoder, transformer_notes, transformer_visits,transformer_with_conv, linear_layer):
    base_encoder.eval()
    transformer_notes.eval()
    transformer_visits.eval()
    transformer_with_conv.eval()
    linear_layer.eval()
    ...
    with torch.no_grad():
        for pid in patient_ids:
            ...
            tv = tv[:, :d]  # (T, d)
            Z0 = torch.cat((tv.unsqueeze(0), tn.unsqueeze(0)), dim=-1)
            ZL = transformer_with_conv(Z0)
            H_prime, U_prime = torch.split(ZL, [d, d], dim=-1)
            H_batch = H_prime.unsqueeze(0)  # (1, T, d)
            U_batch = U_prime.unsqueeze(0)  # (1, T, d)
...
def train_amlm_phase(patient_visits, patient_notes, time_diffs, code_map, vocab, d, device, num_epochs, base_encoder, transformer_notes, transformer_visits,transformer_with_conv):
    base_encoder.train()
    transformer_notes.train()
    transformer_visits.train()
    transformer_with_conv.train()
    optimizer = optim.Adam(
        list(base_encoder.parameters()) +
        list(transformer_notes.parameters()) +
        list(transformer_visits.parameters()),
    ...
    for epoch in range(num_epochs):
        ...
        for pid in patient_ids:
            ...
            Lc, Ln = mlm_loss_patient(H_trans, U_trans, transformer_with_conv,mask_ratio=0.15, num_heads=8, L=4)
...
def train_model(patient_visits, patient_notes, time_diffs, code_map, vocab, d, device, patient_labels, num_epochs, base_encoder, transformer_notes, transformer_visits,transformer_with_conv, linear_layer, batch_size=4):
    icd_dim = len(code_map)
    note_dim = len(vocab)
    base_encoder.train()
    transformer_notes.train()
        ...
        for i in range(0, len(patient_ids), batch_size):
            ...
            for pid in batch_ids:
                ...
                if d_H != d_U:
                    ...
                    d = d_H
                Z0 = torch.cat((H.unsqueeze(0), U.unsqueeze(0)), dim=-1)
                ZL = transformer_with_conv(Z0)
                H_prime, U_prime = torch.split(ZL, [d,d], dim=-1)
                # print(tv.shape)
                # print(U_prime.shape)

                joint_embeddings[pid] = {"note_embedding": U_prime.squeeze(0), "visit_embedding": H_prime.squeeze(0)}
...
def main():
    ...
    transformer_with_conv=TransformerWithConv(2*d, 8, 4).to(device)
    ...
    train_amlm_phase(train_visits, train_notes, train_time_diffs, code_map, vocab, d, device, num_epochs, base_encoder, transformer_notes, transformer_visits,transformer_with_conv)
    print("AMLM-based training phase complete.")
    
    print("Starting train_model...")
    train_model(train_visits, train_notes, train_time_diffs, code_map, vocab, d, device, train_labels, num_epochs, base_encoder, transformer_notes, transformer_visits, transformer_with_conv,linear_layer, batch_size=4)
    ...
    micro_auprc, macro_auprc = calculate_auprc_55(eval_y_pred_probs, eval_y_true)

    print(micro_auroc,macro_auroc)
    print(micro_auprc,macro_auprc)
